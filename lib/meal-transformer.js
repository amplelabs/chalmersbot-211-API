...
const parseOperationHours = (operationHours) => {
    ...
    _.forEach(tokens, (token) => {
        ...
        if (!_.isUndefined(parsedWeekDay)) {
            weekdays.push(parsedWeekDay.slice(0,3)); // only the first 3 char
            return;
        }
    ...
    weekdays = [ ... new Set(weekdays)]; // remove duplicates
...
const resourcesToMeal = (resources) => {
    ...
    _.forEach(resources, (resource) => {
        try {
            ...
            mealSkill.latitude = _.get(resource, 'latitude');
            mealSkill.longitude = _.get(resource, 'longitude');
...
const testCreateMealSkill = () => {
    return new Promise((resolve, reject) => {
        fs.readFile('meal211Toronto_1.json', (err, data) => {
            if (err) {
                return reject(err);
            }
            const mealSkills = resourcesToMeal(JSON.parse(data));
